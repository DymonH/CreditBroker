...
namespace Hlopov.Common.HttpClient
{
    public class ApiBuilder
    {
        ...
        {
            var queries = HttpUtility.ParseQueryString(_builder.Query);
            queries[name] = string.IsNullOrEmpty(queries[name])
                ? value
                : string.Concat(queries[name], ",", value);

            _builder.Query = queries.ToString();
